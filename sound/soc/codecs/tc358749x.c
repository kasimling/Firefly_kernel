...
{
	switch (reg) {
	case TC358749X_CHIPID:
	case TC358749X_REG_CON:
...
/*
static unsigned int tc358749x_read(struct i2c_client *i2c, unsigned int r)
{
        struct tc358749x_priv *tc358749x = i2c_get_clientdata(i2c);
        unsigned int val;
        int ret;

        ret = regmap_read(tc358749x->regmap, r, &val);  
        if (ret < 0)
                return -1;

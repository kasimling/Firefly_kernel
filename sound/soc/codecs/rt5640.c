			   ...
			   struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] = rt5640->asrc_en;
			   ...
			   struct snd_ctl_elem_value *ucontrol)
{
	...
	if (rt5640->asrc_en) {
	regmap_update_bits(rt5640->regmap, 0x0d, //no diff mode
				1<<7, 0<<7);
	regmap_write(rt5640->regmap, 0xfa, 0x3701); //0xfah
	regmap_write(rt5640->regmap, 0x6a, 0x00b4); 
	regmap_write(rt5640->regmap, 0x6c, 0x0800); 
	} else {
	regmap_write(rt5640->regmap, RT5640_PWR_ANLG2, 0x0000); //0x64h
	regmap_write(rt5640->regmap, RT5640_PWR_MIXER, 0x0000);
	regmap_write(rt5640->regmap, RT5640_PWR_VOL, 0x0000);
        regmap_update_bits(rt5640->regmap, RT5640_OUT_R3_MIXER,    //52 mute recmixr
                                1<<3 , 1<<3);
	}
printk("%s: hdmiin audio [%d]\n", __func__, rt5640->asrc_en);
	return 0;
}
...
static const char * const rt5640_asrc_mode[] = {"Disable", "Enable"};

static const SOC_ENUM_SINGLE_DECL(rt5640_asrc_enum, 0, 0,
		rt5640_asrc_mode);
...
static const struct snd_kcontrol_new rt5640_snd_controls[] = {
	...
	SOC_DOUBLE_TLV("ADC Boost Gain", RT5640_ADC_BST_VOL,
			...
			3, 0, adc_bst_tlv),
	/* RT5651 ASRC Switch */
	SOC_ENUM_EXT("RT5640 ASRC Switch", rt5640_asrc_enum,
